...
class PagoController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request)
        {
            ...
            ->select('idPago','paciente.nombre','paciente.apellido','paciente.fechaNacimiento','tipoConsulta.nombreConsulta','estado','total','fechaEmitido')
    ...
    public function show()
    {
       ...
       // $mes=$dato[1]; 
        ...
        $ingreso=DB::table('pago')->select(DB::raw('sum(total) as totalI'))->where('estado', '=', 'Cancelado')->WhereMonth(('fechaEmitido'),'=', $mesA)->WhereYear(('fechaEmitido'), '=', $anioA)->first();
        ...
        $pastel=DB::select("SELECT count('idPago') as cant, tipoConsulta.nombreConsulta as tCons  FROM pago inner join tipoConsulta on pago.idTipoConsulta= tipoConsulta.idTipoConsulta where estado= 'Cancelado' /*and month(fechaEmitido)= $mesA and year(fechaEmitido)= $anioA */ GROUP BY tCons");
/**
        $primer_dia=1;
